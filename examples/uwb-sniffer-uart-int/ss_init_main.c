...
/* For binary to string conversion */
const unsigned char hex_str[]= "0123456789abcdef";


// -------- DOUBLE BUFFER ------------

/* Buffers to store received response message. */
#define RX_BUF_LEN 127  /* 802.15.4 UWB standard maximum frame length. */
#define NUM_BUFS 200 /* Max no. messages that can be stored */
#define NUM_BUFS_MINUSONE 199

static uint8 rx_buffers[NUM_BUFS][RX_BUF_LEN];
static volatile int buf_idx = 0; // which one it is being written to
static int curr_idx = 0;  // which one is currently being processed
#define increment_buf_idx(buf_idx) (((buf_idx) == (NUM_BUFS_MINUSONE)) ? (0) : ((buf_idx)+1))

/*volatile */static bool rx_processing_overrun = false;

/* Hold copies of RX data here for reference. */
volatile static uint32 status_regs[NUM_BUFS]; /* Status register state */
static uint32 rx_len[NUM_BUFS]; /* Length of frame data */
...
int dblbuf_run(void)
{
  while (true) {
    ...
    {
      // Process data
      char rx_data[RX_BUF_LEN*2]; // 1 byte = 2 hex characters
      uint16 i;
      // convert bytes to hex string
      for (i = 0; i < rx_len[curr_idx]; ++i) {
        // convert each byte to hex characters
        rx_data[i*2] = hex_str[(rx_buffers[curr_idx][i] >> 4) & 0x0F];
        rx_data[i*2+1] = hex_str[rx_buffers[curr_idx][i] & 0x0F];
      }
      rx_data[i*2] = '\0';  // null
      // write to UART
      printf("%s\r\n", rx_data);
      // done processing, move to next data
      curr_idx = increment_buf_idx(curr_idx);
    }
  }
}
...
void rx_ok_cb(const dwt_cb_data_t *cb_data)
{
  ...
  uint16 data_len = cb_data->datalength; // read length first

  /* TESTING BREAKPOINT LOCATION #1 */

  /* A frame has been received, copy it to our local buffer. */
  dwt_readrxdata(rx_buffers[buf_idx], data_len, 0);
  rx_len[buf_idx] = data_len;

  buf_idx = increment_buf_idx(buf_idx);//incr_buf_idx;
